...
def detect_chapter(line: str, max_chapter_num=1000):
...
def parse_chapter_id(chapter_id: str) -> List[int]:
    ...
    # 数字章节 1.2.3.，要兼容1-的形式
    if re.fullmatch(r'\d+(?:-\d+)*-?\s*$', chapter_id):
        return [int(p) for p in chapter_id.rstrip('-').split('-')]
    if re.fullmatch(r'\d+(?:\.\d+)*\.?$', chapter_id):
        return [int(p) for p in chapter_id.rstrip('.').split('.')]
...
def find_longest_chapter_chain_with_append(chapters: List[Dict]) -> List[Dict]:
    ...
    for i in range(n):
        ...
        # 过滤明显的测量单位、纯数字等
        if re.search(r'\b\d+\s*(cm|mm|m|kg|g|°C|°F|%)\b', chapter_text, re.I):
            continue
    ...
    # 第二步：在有效章节中构建最长链，但使用更宽松的跳跃检查
    dp = [1] * len(valid_indices)
    prev = [-1] * len(valid_indices)
    max_len = 0
    max_idx = -1
    ...
    for i in range(len(valid_indices)):
        ...
        for j in range(i):
            prev_idx = valid_indices[j]
            prev_parsed = parsed_ids[prev_idx]
            
            if (is_chapter_a_before_b(prev_parsed, curr_parsed) and 
                is_reasonable_chapter_jump(prev_parsed, curr_parsed)):
                if dp[j] + 1 > dp[i]:
                    dp[i] = dp[j] + 1
                    prev[i] = j
    ...
    # 第三步：如果没有找到合理的链，退回到简单的顺序过滤
    if max_len < 2:
        # 简单按章节编号顺序过滤
        return simple_chapter_filter(chapters)
...
def simple_chapter_filter(chapters: List[Dict]) -> List[Dict]:
    ...
    for i, chap in enumerate(chapters):
        ...
        if first_num >= ALPHA_OFFSET:  # 附录章节
            first_num -= ALPHA_OFFSET
        
        if 1 <= first_num <= 20:  # 合理的章节范围
            result.append(chap)
        else:
            # 不合理的章节，追加到上一个有效章节
            if result:
...
def process_sections_with_lis(chapters):
    ...
    processed_sections = []
    for sec in sections:
        valid_chaps = find_longest_chapter_chain_with_append(sec["chapters"])
...
def filter_start_of_main(chapters: List[Dict]) -> Tuple[List[Dict], str]:
    ...
    for i, chap in enumerate(chapters):
        # 正文主链或附件内部章节
        if chap.get("chapter_id", "") in {"1", "1-", "1."}:
            # SCOPE / GENERAL / INTRO 等都算正文起点
            title_upper = chap.get("chapter_title", "").upper()
            if any(k in title_upper for k in ["SCOPE", "GENERAL", "INTRO", "总则", "范围"]):
                start_index = i
                # break
...
def parse_pdf_to_chapter_tree(pdf_path: str) -> Tuple[List[Dict], Dict[str, str]]:
    ...
    for top_sec in attachment_sections:
        ...
        for sec in sections:
            ...
            # 3️⃣ 对每个部分内部保留最长链
            valid_chaps_in_sec = find_longest_chapter_chain_with_append(filtered_chapters)
            tree_in_sec = build_tree(valid_chaps_in_sec)
            build_full_path(tree_in_sec)
            # 插入键值对 section
            section_tree_list.append({
                "section": sec["section"],
                "context": skipped_text + filtered_chapters[0].get('raw_text',''),
