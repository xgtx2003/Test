...
def detect_chapter(line: str, max_chapter_num=1000, language='en', number_analysis=None):
    clean_line = line.strip() # 去除掉空格，仅保留文本
    if not clean_line:
        return None

    for pattern in chapter_patterns:
        m = pattern.match(clean_line) # 先匹配章节模式
        if m:
            # 提取了id和标题
...
def build_term_dict(raw_text: str) -> Dict[str, str]:
    ...
    for m in pattern.finditer(text):
        cn = m.group("cn").strip() # 查找中文
        en = m.group("en").strip() # 查找英文
        if cn and en:
            term_map[cn] = en
    return term_map
...
# 判断是否需要把当前行合并到上一行（标题 or 视觉上判断距离）
...
def extract_full_text_with_filter(pdf_path: str, top_crop=0.08, bottom_crop=0.08): # 大改动
    ...
    return normalized  # 返回格式：[(处理后文本, 页码), ...]


# 检测输入文本的章节模式（字母前 or 数字前）
...
# 分析章节编号的数字分布，确定合理的数字范围（处理文档解析过程中的各种数字问题）
...
# 简单的章节过滤环节，最长链算法失效时的备用方案（尽可能构建合理的章节结构）
...
def parse_pdf_to_chapter_tree(pdf_path: str) -> Tuple[List[Dict], Dict[str, str]]: # 大改动
    """
    从 PDF 中提取章节树和术语映射，新增章节页码范围和表格关联
    ...
    for table in tables:
        # 处理每个表格的逻辑
        print(f"表格: {table['table_id']} (页码: {table['page_num']})")
...
# 从PDF中提取所有表格及其标识（后续进行表格的处理）
def extract_tables_from_pdf(pdf_path: str) -> List[Dict]: # 大改动
    """
...
def main():
    ...
    parser.add_argument("--pdf_path", default="D:/汽车检测知识图谱Agent/汽车检测知识图谱Agent/标准差异分析/GSO-1040/GSO-1040-2000_OCR/GSO-1040-2000-E机动车- 轻型柴油引擎车辆大气污染物排放允许限值.pdf") # 输入路径
    ...
    parser.add_argument("--output", help="输出 JSON 文件路径", default="outputTest2.json") # 输出路径
